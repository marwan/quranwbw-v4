<script>
	...
	$: if ($page.url.href || $__pageURL || $__chapterNumber || $__pageNumber) __quranNavigationDrawerHidden.set(true);
	...
	$: {
		if ($__quranNavigationDrawerHidden) {
			keyPages = (async () => {
...
<Drawer divClass="overflow-y-auto z-50 p-0 bg-white rounded-b-3xl" placement="top" width="w-full" transitionType="fly" transitionParams={transitionParamsTop} bind:hidden={$__quranNavigationDrawerHidden} id="sidebar7">
	<div class="flex flex-col space-y-2 justify-between max-w-screen-lg px-4 py-5 mx-auto">
		...
		<div class="w-full px-2">
			<button class="w-full {buttonClasses}" on:click={() => __quranNavigationDrawerHidden.set(true)}>Close</button>
		</div>
	</div>
</Drawer>

<Modal id="quranNavigationModal" title="Navigate" size="sm" class="rounded-3xl text-black theme" bodyClass="p-2 !border-t-0" headerClass="hidden" placement="center">
	<div class="flex flex-col space-y-2 justify-between max-w-screen-lg px-4 py-5 mx-auto">
		<div class="mx-2">
			<div id="navigatation-inputs" class="flex flex-col space-y-4 mb-4 justify-start theme-grayscale">
				<div class="flex flex-row w-full h-fit items-center">
