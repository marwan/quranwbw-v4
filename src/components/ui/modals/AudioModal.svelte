<script>
  ...
  $: audioSettingsJSON = JSON.stringify($__audioSettings, undefined, 2);
</script>

<!-- Audio modal -->
<div id="audioModal" data-chapter data-verse tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full grayscale">
  <div class="relative w-full max-w-md max-h-full">
    ...
    <div class="relative bg-white rounded-lg shadow dark:bg-slate-800">
      ...
      <div class="px-6 py-6 lg:px-8">
        <h3 id="audioModal-title" class="mb-4 text-xl font-medium text-gray-900 dark:text-slate-400">{quranMetaData[$__audioSettings.playingChapter || 1].transliteration}, {$__audioSettings.playingKey}</h3>
