...
<!-- Notes modal -->
<div id="notes-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full theme-grayscale">
	<div class="relative w-full max-w-md max-h-full">
		<!-- Modal content -->
		<div class="relative bg-white rounded-3xl shadow daaark:bg-gray-700">
			<button type="button" on:click={() => toggleModal('notes-modal', 'hide')} class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-[#ebebeb] hover:text-gray-900 rounded-3xl text-sm w-8 h-8 ml-auto inline-flex justify-center items-center daaark:hover:bg-[#ebebeb] daaark:hover:text-white" data-modal-hide="notes-modal">
