<script>
	...
	const innerContainerClasses = 'overflow-y-auto max-h-screen pr-2 pb-28';
	...
	function goBackToMainSettings() {
		// we set the scroll pos of individual-setting block to top so if the user selects another individual settings, it's already scrolled to top
		try {
			document.getElementById('individual-setting').scrollTop = 0;
		} catch (error) {}
...
<!-- settings drawer -->
<Drawer placement="right" transitionType="fly" transitionParams={transitionParamsRight} bind:hidden={$__settingsDrawerHidden} class="theme w-full md:w-1/2 lg:w-[430px] md:rounded-tl-3xl md:rounded-bl-3xl pt-0 overflow-y-hidden" id="settings-drawer">
	<div class="flex z-30 top-0 sticky bg-white border-b-2 border-black/10 mb-4">
		...
		{:else if individualSettingsVisible}
	...
	<!-- all-settings -->
	<div id="all-settings" class="{innerContainerClasses} {allSettingsVisible ? 'block' : 'hidden'}">
		<!-- <div class="flex z-30 top-0 sticky bg-white border-b-2 border-black/10 mb-4">
				<h5 id="drawer-label" class="inline-flex items-center my-4 text-3xl font-semibold">Settings</h5>
				<CloseButton on:click={() => ($__settingsDrawerHidden = true)} class="my-4 rounded-3xl" />
			</div> -->
	...
	<!-- individual-setting -->
	<div id="individual-setting" class="{innerContainerClasses} {individualSettingsVisible ? 'block' : 'hidden'}">
