<script>
	...
	$: navbarClasses = `bg-white fixed w-full z-20 top-0 left-0 border-b border-blackkk/10 text-black print:hidden theme ${$__currentPage === 'home' ? 'hidden' : 'block'}`;
...
<nav id="navbar" class={navbarClasses}>
	<div id="top-nav" class={topNavClasses} aria-label="Home">
		<a href="/" class="flex flex-row items-center p-3 cursor-pointer hover:bg-lightGrayyy rounded-3xl" aria-label="Home">
			<Home />
		...
		<button class="flex flex-row items-center p-3 cursor-pointer hover:bg-lightGrayyy rounded-3xl" type="button" aria-label="Menu" on:click={() => __siteNavigationModalVisible.set(true)}>
	...
	{#if $__currentPage === 'chapter'}
		<div id="bottom-nav" class="flex flex-row items-center justify-between border-t border-blackkk/10 text-xs max-w-screen-lg mx-auto px-6 theme-grayyyscale">
			...
			<div class="flex flex-row items-center py-2">
				...
				<span class="px-1 opacityyy-60">/</span>
				<span>{lastReadJuz ? `${term('juz')} ${lastReadJuz}` : '...'}</span>
			</div>
		</div>

		<div id="chapter-progress-bar" class="fixed inset-x-0 z-20 h-1 bg-blackkk/30 rounded-r-3xl" style="width: {chapterProgress}%" />
	{/if}
