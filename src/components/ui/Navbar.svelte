...
<nav id="navbar" class="{$__currentPage === 'home' ? 'hidden' : 'block'} bg-white fixed w-full z-20 top-0 left-0 border-b border-gray-200 text-black print:hidden theme-grayscale">
	<div id="top-nav" class="{$__topNavbarVisible === true ? 'block' : 'hidden'} flex flex-row items-center justify-between max-w-screen-lg mx-auto px-4 py-2">
		<Link to="/" class="flex flex-row items-center p-3 cursor-pointer hover:bg-[#ebebeb] rounded-lg">
			<Home />
		...
		<button id="navigationDropdownButton" data-dropdown-toggle="navigationDropdown1" class="{$__currentPage === 'chapter' ? 'block' : 'hidden'} flex items-center p-3 text-sm border-gray-200 w-auto p-2 hover:bg-[#ebebeb] rounded-lg">
			{@html navbarChapterName}
			<ChevronDown />
		</button>

		<!-- chapter, verse, page navigation -->
		<Dropdown bind:open={navigationDropdownOpen}>
		...
		<button class="{$__currentPage !== 'chapter' ? 'block' : 'hidden'} flex items-center p-3 text-sm border-gray-200 w-auto p-2 hover:bg-[#ebebeb] rounded-lg">
		...
		<Dropdown>
			...
			<DropdownItem on:click={() => toggleModal('token-modal', 'show')} class="text-xs">Token Login</DropdownItem>

			<a href="https://legacy.quranwbw.com/" target="_blank">
				<DropdownItem class="text-xs">Legacy Website</DropdownItem>
			</a>
		</Dropdown>
	</div>
