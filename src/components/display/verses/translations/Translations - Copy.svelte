<script>
	...
	console.log(`${value.meta.chapter}:${value.meta.verse}`);
	...
	<div class={verseTranslationClasses} data-fontSize={fontSizes.verseTranslationText}>
		...
		{:else}
			...
			{:then verseTranslationData}
				...
				<!-- {#if chapterData !== null}
					{#if $__verseTranslations.includes(1)}
						{console.log(`inside: ${value.meta.chapter}:${value.meta.verse}`, chapterData)}
						<Layout verseTranslationID={1} verseTranslation={chapterData[`${value.meta.chapter}:${value.meta.verse}`].translations[0]} {value} />
					{/if}
				...
				<!-- <Layout verseTranslationID={1} verseTranslation={chapterData[0].translations[0]} {value} /> -->

				<!-- {#if chapterData !== null}
					{#if chapterData[`${value.meta.chapter}:${value.meta.verse}`].hasOwnProperty('translations')}
						{#each Object.entries(chapterData[`${value.meta.chapter}:${value.meta.verse}`].translations) as [verseTranslationID, verseTranslation]}
							<Layout verseTranslationID={verseTranslation.resource_id} {verseTranslation} {value} />
						{/each}
					{/if}
				{/if} -->
